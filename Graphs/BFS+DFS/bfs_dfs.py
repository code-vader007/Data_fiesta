...
class Graph:
    ...
    def bfs2(self,vertex):
        ...
        while not layer.isEmpty():
            a=layer.popfront()
            print(a)
            aidx = self.vertexList.index(a)
            loc = self.vertexList.locate(a)
            edgelist = loc.getEdges()
            if edgelist != None:
                for b in edgelist:
                    idx = self.vertexList.index(b)
                    if parent[idx] == None:
                        layer.pushback(b)
                        distance[idx] = distance[aidx] + 1
                        parent[idx] = a
        return (distance,parent)
...
class Stack:
    ...
    def isEmpty(self):
        if self.numItems != 0:
            return False
        return True
edges = [(1,3),(1,4),(3,5),(3,4),(5,1),(2,4),(3,1),(2,5),(6,4)]
g = Graph(edges)
print(g.bfs(3))
print(g.bfs2(3))
